...
describe('Testing order API endpoints', () => {
    ...
    it('[PUT] to /api/orders/:id should update the order object and return the object and a 200', async () => {
        const res = await request.put('/api/orders/1').send({ id: 1, status: 'completed', user_id: '1' }).set('Accept', 'application/json').set('Authorization', `Bearer ${adminToken}`)
        expect(res.status).toBe(200)
        expect(res.body).toEqual(jasmine.objectContaining({ id: 1, status: 'completed', user_id: '1' }))
    })
})
