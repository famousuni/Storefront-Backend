...
describe('Testing user API endpoints', () => {
    ...
    it('[GET] to /api/users/:id should get the user information for the id presented and return a 200', async ()=> {
        ...
        expect(res.status).toBe(200)
        expect(res.body).toEqual(jasmine.objectContaining({'id': 2, 'username': 'testcustomer', 'firstname': 'testfirstname', 'lastname': 'testlastname', 'role': 'customer'}))
    })
    ...
    it('[PUT] to /api/users/:id should update your user and return a 200 and user object', async () => {
        ...
        expect(res.status).toBe(200)
        expect(res.body).toEqual(jasmine.objectContaining({'id': 2, 'username': 'testcustomer', 'firstname': 'custupdatedfirstname', 'lastname': 'testlastname', 'role': 'customer'}))
    ...
    it('[PUT] to /api/users/:id by admin can update any user and return a 200 and the user object', async () => {
        ...
        const res = await request.put('/api/users/2').send(user).set('Accept', 'application/json').set('Authorization', `Bearer ${customerToken}`)
        expect(res.status).toBe(200)
        expect(res.body).toEqual(jasmine.objectContaining({'id': 2, 'username': 'testcustomer', 'firstname': 'custupdatedfirstname', 'lastname': 'adminupdatedlastname', 'role': 'customer'}))
