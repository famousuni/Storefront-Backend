...
describe('User Model', () => {
  ...
  it('create method should add a user', async () => {
    ...
    expect(result).toEqual(
      jasmine.objectContaining({ username: 'testuser', role: 'admin' })
    )
    result = await store.create(user2)
    expect(result).toEqual(
      jasmine.objectContaining({ username: 'testuser2', role: 'customer' })
    )
  })
  ...
  it('show method should return the correct user', async () => {
    ...
    expect(result).toEqual(
      jasmine.objectContaining({ id: 6, username: 'testuser', role: 'admin' })
    )
  })
