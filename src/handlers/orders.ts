...
const show = async (req: Request, res: Response) => {
  try {
    ...
    if (
      ...
      throw new Error('customers can only show their own orders')
...
const update = async (req: Request, res: Response) => {
  try {
    if (
      ...
      throw new Error('customers can only update their own orders')
    if (res.locals.role !== 'customer' && res.locals.role !== 'admin')
      throw new Error('invalid role')
...
const addProduct = async (_req: Request, res: Response) => {
  //TODO: users should only add products to their own orders
  try {
